...
def create_app():
    ...
    #This route will load the upload page, which is a reference to validate data pipelines independent of the front end being online. Handy for Local Testing for the DS Team.
    def upload():
        return render_template('base.html')

    @application.route("/bulk_upload", methods=['GET'])
    #This route is used for bulk upload and processing of images to bootstrap our model, will take hours to run given current amount of data, but if there is any alterations to the image processing pipeline, it will be required to uniformly train the model on the changes.
    def bulk_upload():
    ...
    #8b. For DS Team Use: Use the commented out return argument at the end of this function if you wish to use the DS page, which will display data from intermediary steps of this pipeline. Very handy for DS team testing, to locally validate changes.
    def demo_file():
        #Pulls the image file in the post request as a object in memory
        image_file = request.files['file']
        #Takes the hash of the image - it will be used as a unique identifer across the DS API.
        ...
        #processes the raw image, and returns the raw image to processed variable and returns 81 numpy arrays (of 28x28 dimensions) representing each individual sudoku cell that makes up the whole puzzle
        processed, imgarray = pipeline(imgurl)
        processed_image = Image.fromarray(processed)

        #loads the processed image in memory so it can be uploaded to S3 in the process_puzzles folder.
        ...
        #This will add sevearl seconds to response time, so consider commenting out when going to production once this is automated into a daily batch job across newly uploaded images.
        processed_cells = []
        i = 0
        j = 0
        ...
        #loads it into a csv_buffer object and loaded to S3 for sagemaker processing.
        ...
        #Sends an POST request to the sagemaker API URL, appending the created above to the request for the sagemaker endpoint to read in.
        ...
        # The web team needs to endpoint for reliability of testing, so leaving it uncommented. A future release should use the sagemaker endpoint in production.
        pred = predict_knn(config('MODEL_FILEPATH'), imgarray)

        #runs the predicted digits against the solver function
        #The definitions of these paramaters is documented in solver.py and ai.py.
        grid_status = solve(str(pred))[0]
        solution = solve(str(pred))[1]
        difficulty = solve(str(pred))[3]

        #In the event a puzzle has invalid values, remapping from backend to frontend is required for the front end to highlight invalid cells (web team did not get this functionality in final launch, but leaving it in for a future release as DS team functionality is ready)
        ...
        #If the puzzle is solveable, adding the numpy arrays for the sudoku cells into the ModelTrainer Database for future image training.
        if grid_status == 1:
        ...
        #This is the production API response to use for web team functionality.
    ...
    #This route was handy when the team took 500+ pictures of sudoku puzzles to help bootstrap training our own model.
    def bulk_processing():
        ...
        #loops through each image organized above and processed using the pipeline function, then subdividies it into 81 sudoku cells and uploads each cell.
    ...
    def reset():
        #sets the .csv of reference sudoku puzzles and populates it into a Postgres database called PuzzleTable.
    ...
    def train_model():
        ...
        # First column is predicted outcome, then 784 columns for each value in the 28x28 Numpy Array representing each Sudoku cell image.
        ...
        #removing wrapper brackets, and reverses the . character to 0 for model training
        df['numpy_array'] = df['numpy_array'].str.strip("}")
        df['numpy_array'] = df['numpy_array'].str.strip("{")
        df['predicted_value'] = df['predicted_value'].replace(".", "0")

        #for each row in the dataframe, expands out the Numpy Array to the same format as the Sagemaker endpoint.
        pixel_values = []
        ...
        #assigning the predicted digit to the first column of the final dataframe
