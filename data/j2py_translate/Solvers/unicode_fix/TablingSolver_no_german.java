...
public class TablingSolver extends AbstractSolver {
     ...
     */
    private List<SolutionStep> steps; // gefundene Losungsschritte
    /**
    ...
    private Sudoku2 savedSudoku;            // Sudoku2 im Ausgangszustand (fur Erstellen der Tables)
...
//    private SudokuSet alsBuddies = new SudokuSet(); // cells that can see all the cells of the als
    ...
    private List<SolutionStep> singleSteps = new ArrayList<SolutionStep>();  // fur Naked und Hidden Singles
    private int[] chain = new int[Options.getInstance().getMaxTableEntryLength()]; // globale chain fur buildChain()
    private int chainIndex = 0; // Index des nachsten Elements in chain[]
    private int[][] mins = new int[200][Options.getInstance().getMaxTableEntryLength()]; // globale chains fur networks
    private int[] minIndexes = new int[mins.length]; // Indexe der nachsten Elemente in mins[]
    private int actMin = 0;                          // derzeit aktuelles min
    private int[] tmpChain = new int[Options.getInstance().getMaxTableEntryLength()]; // globale chain fur addChain()
    private Chain[] tmpChains = new Chain[9];
    private int tmpChainsIndex = 0;
    private SudokuSet lassoSet = new SudokuSet();  // fur addChain: enthalt alle Zellen-Indices der Chain
...
//                        if (dest.isExpanded(orgIndex) && dest.getDistance(orgIndex) > (srcBaseDistance + srcDistance)) {
                        if (dest.isExpanded(orgIndex)
                            ...
                            // Alter Eintrag war langer oder komplizierter als neuer -> umschreiben
...
//        }
    ...
    public static void main(String[] args) {
        ...
        // sollte 2 Grouped AICs fur 59 geben:
